...
# Preprocess for fast matching
question_lookup = {}
all_questions = []
for item in qa_data:
    for q in item["questions"]:
        q_lower = q.lower().strip()
        question_lookup[q_lower] = item["answer"]
        all_questions.append(q_lower)
...
def chatbot_response():
    user_input = request.json.get("msg", "").lower().strip()

    # Exact match first
    if user_input in question_lookup:
        return jsonify({"response": question_lookup[user_input]})

    # Fuzzy match for close results
    match, score, _ = process.extractOne(user_input, all_questions, score_cutoff=70)
    if match:
        return jsonify({"response": question_lookup[match]})

    return jsonify({"response": "Answer not available"})
