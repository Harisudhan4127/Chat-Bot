...
def find_best_match(user_input):
    ...
    for entry in qa_data:
        for q in entry["questions"]:
            ...
            question_to_answer[q.lower()] = entry["answer"]

    match = difflib.get_close_matches(user_input_lower, all_questions, n=1, cutoff=0.6)
    return question_to_answer.get(match[0], "Answer not available") if match else "Answer not available"
...
def get_bot_response():
    ...
    return jsonify({"reply": find_best_match(user_message)})
